...
class VideoStreamSubscriber:
    ...
    def receive(self):
        ...
        if not self.queue.empty():
            data_received = self.queue.get(False)
    ...
    def _run(self, queue):
        ...
        while not self._stop:
            if not queue.full():
                sent_from, jpeg_buffer = receiver.recv_jpg()
                img = simplejpeg.decode_jpeg(jpeg_buffer, colorspace='BGR')
                queue.put((sent_from, img), False)
        receiver.close()
